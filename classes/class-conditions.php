...
class westcpa_Conditions {
	...
	public function check( $type, $condition, $post_obj ) {
		...
		if ( empty( $type ) ) {
			...
			$check_type = new WP_Error( 'type_empty', __( 'The type for the metabox is empty.', 'westcpa' ) );

		}

		if ( is_wp_error( $check_type ) ) {

			wp_die( $check_type->get_error_message(), __( 'Type is empty.', 'westcpa' ) );

		}

		if ( empty( $condition ) ) {

			$check_condition = new WP_Error( 'condition_empty', __( 'The condition for the metabox is empty.', 'westcpa' ) );

		}

		if ( is_wp_error( $check_condition ) ) {

			wp_die( $check_condition->get_error_message(), __( 'Condition is empty.', 'westcpa' ) );

		}

		if ( empty( $post_obj ) ) {

			$check_post_obj = new WP_Error( 'post_object_empty', __( 'The post object for the metabox is empty.', 'westcpa' ) );

		}

		if ( is_wp_error( $check_post_obj ) ) {

			wp_die( $check_post_obj->get_error_message(), __( 'Post object is empty.', 'westcpa' ) );
